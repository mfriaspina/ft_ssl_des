...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/02/16 15:39:52 by mfrias            #+#    #+#             */
/*   Updated: 2020/02/17 13:50:00 by mfrias           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_ssl.h"

const static t_ubyte g_pc1[] = {
	57, 49, 41, 33, 25, 17, 9, 1, 58, 50, 42, 34, 26, 18, 10, 2, 59, 51, 43,
	35, 27, 19, 11, 3, 60, 52, 44, 36, 63, 55, 47, 39, 31, 23, 15, 7, 62, 54,
	46, 38, 30, 22, 14, 6, 61, 53, 45, 37, 29, 21, 13, 5, 28, 20, 12, 4};

const static t_ubyte g_pc2[] = {
	14, 17, 11, 24, 1, 5, 3, 28, 15, 6, 21, 10, 23, 19, 12, 4, 26, 8, 16, 7,
	27, 20, 13, 2, 41, 52, 31, 37, 47, 55, 30, 40, 51, 45, 33, 48, 44, 49, 39,
	56, 34, 53, 46, 42, 50, 36, 29, 32 };

const static t_ubyte g_e[] = {
	32, 1, 2, 3, 4, 5, 4, 5, 6, 7, 8, 9, 8, 9, 10, 11, 12, 13, 12, 13, 14, 15,
	16, 17, 16, 17, 18, 19, 20, 21, 20, 21, 22, 23, 24, 25, 24, 25, 26, 27, 28,
	29, 28, 29, 30, 31, 32, 1};

const static t_ubyte g_s[][64] = {
	{
...
const static t_ubyte g_p[] = {
	16, 7, 20, 21, 29, 12, 28, 17, 1, 15, 23, 26, 5, 18, 31, 10, 2, 8, 24, 14,
	32, 27, 3, 9, 19, 13, 30, 6, 22, 11, 4, 25};

const static t_ubyte g_shifts[] = {
	1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1};

void		sub_keys_two(t_ubyte c[17][7], t_ubyte d[17][4], t_subkey ks)
{
	int	j;
	int	i;
	...
	while (++i < 17)
	{
		j = 27;
		while (++j < 56)
			poke_bit(c[i], j, peek_bit(d[i], j - 28));
	}
	i = 0;
	while (++i < 17)
	{
		j = -1;
		while (++j < 48)
			poke_bit(ks[i], j, peek_bit(c[i], g_pc2[j] - 1));
	}
}
...
void		get_sub_keys(const t_ubyte *key, t_subkey ks)
{
	...
	ft_memset(ks, 0, sizeof(t_subkey));
	i = -1;
	while (++i < 56)
		poke_bit(kp, i, peek_bit(key, g_pc1[i] - 1));
	i = -1;
	while (++i < 28)
	{
		poke_bit(c[0], i, peek_bit(kp, i));
		poke_bit(d[0], i, peek_bit(kp, i + 28));
	}
	i = 0;
	while (++i < 17)
	{
		shift_left(c[i - 1], 28, g_shifts[i - 1], c[i]);
		shift_left(d[i - 1], 28, g_shifts[i - 1], d[i]);
	}
	sub_keys_two(c, d, ks);
}

void		f_cycle(t_ubyte *er, t_ubyte *sr, int i)
{
	int b[6];
	int k;
	int	m;
	...
	k = -1;
	while (++k < 6)
		b[k] = peek_bit(er, (i * 6) + k) != 0 ? 1 : 0;
	m = g_s[i][((2 * b[0] + b[5]) * 16)
		+ 8 * b[1] + 4 * b[2] + 2 * b[3] + b[4]];
	k = 1;
	while (m > 0)
	{
		p = m % 2;
		poke_bit(sr, (i + 1) * 4 - k, p == 1);
		m /= 2;
		k++;
	}
}
...
void		f(t_ubyte *r, t_ubyte *ks, t_ubyte *sp)
{
	...
	i = -1;
	while (++i < 48)
		poke_bit(er, i, peek_bit(r, g_e[i] - 1));
	i = -1;
	while (++i < 6)
		er[i] ^= ks[i];
	i = -1;
	...
	i = -1;
	while (++i < 32)
		poke_bit(sp, i, peek_bit(sr, g_p[i] - 1));
}
